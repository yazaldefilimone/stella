...
impl<'a> Checker<'a> {
  pub fn declare_variables(&mut self, names: NameType, ty: Type, local: bool, loc: Location) -> Result<(), Diagnostic> {
    match ty {
      Type::Grup(group) => {
        for (index, token) in names.iter().enumerate() {
          let assigned_type = group.types.get(index).cloned().unwrap_or(Type::Nil);
          self.declare_variable(token, assigned_type, local, &loc)?;
        }
      }
      _ => {
        for (index, token) in names.iter().enumerate() {
  ...
  pub fn declare_variable(
    ...
    if let Some(value_type) = &value.1 {
      ...
      if !checked_value_type.check_match(&checked_type) {
        return Err(self.create_type_mismatch(checked_value_type, checked_type, value.0.location.clone()));
      }
      ...
      match (&checked_type, &checked_value_type) {
        (Type::Function(expected), Type::Function(found)) => {
          self.bind_function(expected, found, loc)?;
    ...
    self.emmit_shadowing_and_redeclaration(lexeme, local, &value.0.location)?;
    ...
    if local {
      if self.ctx.defined_in_current_scope(lexeme) {
        if self.ctx.lookup_local_variable(lexeme) {
          return Err(self.create_redeclaration(lexeme, location.clone()));
        } else {
          let diagnostic = TypeWarning::ShadowedVariable(lexeme.to_string(), Some(location.clone()));
      ...
      if !previous_type.check_match(&current_type) {
        return Err(self.create_type_mismatch(previous_type.to_owned(), current_type.to_owned(), location.clone()));
      }
    }
    Ok(())
  }
  ...
  pub fn bind_function(
    ...
    if expected.params.len() != found.params.len() {
      return Err(self.create_function_arity_mismatch(expected.params.len(), found.params.len(), loc.clone()));
    }
    ...
    for (expected_param, found_param) in expected.params.iter().zip(found.params.iter()) {
      ...
      if !expected_param.check_match(found_param) {
        return Err(self.create_type_mismatch(expected_param.to_owned(), found_param.to_owned(), loc.clone()));
      }
    }
    ...
    if !expected_return_type.check_match(&found_return_type) {
      return Err(self.create_type_mismatch(expected_return_type, found_return_type, loc.clone()));
    }
    return Ok(());
  }
}
